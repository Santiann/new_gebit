...
class Register
{
    ...
    public static function registerOnSystem($requestObj)
    {
        ...
        if(($requestData['a005_cnpj']??"") == '')
        {
            ...
            $requestData["a005_nome_fantasia"] = $requestData['a005_nome_fantasia'] ?? $requestData['name'];
            $requestData["a005_razao_social"] = $requestData['a005_razao_social'] ?? $requestData['name'];
        ...
        try {
            ...
            if (!$usuario) {
                $usuario = new Usuario();
                $usuario->a001_nome = $requestData['a001_nome_completo']??($requestData["nome"])?? $requestData['name'];
                $usuario->a001_email = $requestData['a001_email'];
                $usuario->a001_cargo = $requestData['a001_cargo'] ?? null;
                $usuario->a001_cep = isset($requestData['a001_cep']) ? str_replace('-', '', $requestData['a001_cep']) : null;
                $usuario->a001_telefone = isset($requestData['a001_telefone']) ? str_replace(['(', ')', ' '], '', $requestData['a001_telefone']) : null;
                $usuario->a001_endereco = isset($requestData['a001_endereco']) ? $requestData['a001_endereco'] : null;
                $usuario->a001_numero_end = isset($requestData['a001_numero_end']) ? $requestData['a001_numero_end'] : null;
                $usuario->a047_id_cidade = isset($requestData['a001_id_cidade']) ? $requestData['a001_id_cidade'] : null;
                $usuario->a001_complemento = isset($requestData['a001_complemento_end']) ? $requestData['a001_complemento_end'] : null;
                $usuario->a001_bairro = isset($requestData['a001_bairro']) ? $requestData['a001_bairro'] : null;
            ...
            if (!$user) {
                $user = new User();
                $user->name = $requestData['a001_nome_completo']??($requestData["name"]);
                $user->email = $requestData['a001_email'];
                $user->password = $requestData['password'] ?? Str::random(20);
            ...
            $requestData['a047_id_cidade'] = $requestData['a005_id_cidade'] ?? 1;
