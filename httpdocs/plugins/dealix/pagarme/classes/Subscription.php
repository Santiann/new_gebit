...
class Subscription
{
    ...
    public function create($data, $user)
    {
        ...
        $userSystem = UserSystem::where('email', \Auth::user()->email ?? $user->a001_email)->first();
        $userSystem->creditos = $plan->credits;
        $userSystem->save();

        $user = User::find(\Auth::user()->id ?? $user->id_site);
    ...
    private function prepareToSubscribe($data, $user)
    {
        ...
        $customer = [
            ...
            'address' => [
                'street' => $user->a001_endereco ?? "Incompleto",
                'street_number' => $user->a001_numero_end ?? "0",
                'complementary' => $user->a001_complemento ?? "",
                'neighborhood' => $user->a001_bairro ?? "Incompleto",
                'zipcode' => $user->a001_cep ?? "01001001",
            ],
            'phone' => [
                'ddd' => $phone['ddd'],
                'number' => $phone['number']
