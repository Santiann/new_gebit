...
class Account extends ComponentBase
{
    ...
    public function onSignin()
    {
        try {
            ...
            */
            ...
            /**
             * criar api_token e redirecionar caso seja usuario sem acesso ao site
             */
            $user_system = \Dealix\Register\Models\User_Auth::where('email', $credentials['login'])->first();

            if ($user_system && $redirect = $this->checkOldUser($user_system))
                return Redirect::away($redirect);

            $this->generateApiToken($user_system);
    ...
    public function onActivate($code = null)
    {
        try {
            ...
            $userSite = UserModel::find($userId);
            $userSystem = \Dealix\Register\Models\User::where('a001_email', $userSite->email)->first();
            $userSystem->id_site = $userId;

            /* efetuando assinatura no pagarme após validar e-mail */
            // $data = [
            //     "payment_method" => "boleto",
            //     "plan_id" => $this->param('plan_id')
            // ];

            // $pagarmeSubscriptions = new \Dealix\Pagarme\Classes\Subscription();
            // $subscription = $pagarmeSubscriptions->create($data, $userSystem);
            // $subscription->api_token  = \Dealix\Register\Models\User_Auth::findByEmail($userSystem->a001_email)->first()->api_token;

            // $subscription = new \stdClass();
            // $subscription->id = 0;
            // $subscription->customer_id = 0;
            // $subscription->plan_id = $this->param('plan_id');
            // $subscription->status = 'trial';
            // $subscription->payment_method = 'trial';

            // if ($subscription) {
            //     $checkout = new \Dealix\Checkout\Components\Checkout(); 
            //     $checkout->registerSubscription($subscription);
            // }
