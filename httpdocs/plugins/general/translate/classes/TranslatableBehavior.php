...
abstract class TranslatableBehavior extends ExtensionBase
{
    ...
    public function initTranslatableContext()
    {
        ...
        $this->translatableContext = $translate->getLocale(!\App::runningInBackend());
    ...
    protected function getAttributeFromData($data, $attribute)
    {
        ...
        $value = array_get($data, implode('.', $keyArray));

        if ($this->model->isJsonable($attribute) && !empty($value)) {
            $_attr = json_decode($value, true);
