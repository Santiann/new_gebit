...
class PageList
{
    ...
    {
        return \Cms\Classes\Page::listInTheme($this->theme, $skipCache);
    }

    /**
     * Transforma um caminho em 'ID'
     * @param string $url
     * @return string
     */
     ...
     * The method uses the theme's meta/backend-menu.yaml file to build the hierarchy. The pages are returned
    ...
    public function getPageTree($skipCache = false)
    {
        ...
        $iterator = function($configPages) use (&$iterator, &$pages) {
            ...
            foreach ($configPages as $fileName => $config) {

                $submodules = [];
                if(isset($config['modules'])){

                    $submodules = $config['modules'];

                    foreach($config['modules'] as $moduleIdentifier => $moduleConfig){
                        $url = array_get($moduleConfig, 'url');
                        $submodules[$moduleIdentifier]['urlId'] = $this->getUrlId($url);
                    }
                }
                ...
                if(isset($config['pages'])){
                    ...
                    $subpages = $config['pages'];
                }

                foreach ($pages as $index => $page) {
                    $pages[$index]->getBaseFileName = function(){
        ...
        return $iterator($config['backend-menu']);
    }
    ...
    public function getPageParent($page)
    {
        ...
        $iterator($pagesConfig['backend-menu']);
    ...
    /**
     * Updates the page hierarchy structure in the theme's meta/backend-menu.yaml file.
    ...
    public function updateStructure($structure)
    {
        ...
        $originalData['backend-menu'] = $structure;
    ...
    public function appendPage($page)
    {
        ...
        $originalData = $this->getPagesConfig();
        $structure = $originalData['backend-menu'];
    ...
    public function removeSubtree($page)
    {
        ...
        $updatedStructure = $iterator($pagesConfig['backend-menu']);
        $this->updateStructure($updatedStructure);
    }

    /**
     * Returns the parsed meta/backend-menu.yaml file contents.
     * @return mixed
     */
    protected function getPagesConfig()
    {
        ...
        if (!file_exists($filePath)) {
            return self::$configCache = ['backend-menu' => []];
        }

        $config = Yaml::parse(File::get($filePath));
        if (!array_key_exists('backend-menu', $config)) {
            throw new SystemException('The content of the theme meta/backend-menu.yaml file is invalid: the "backend-menu" root element is not found.');
        }
    ...
    /**
     * Returns an absolute path to the meta/backend-menu.yaml file.
     * @return string
     */
    protected function getConfigFilePath()
    {
        return $this->theme->getPath().'/meta/backend-menu.yaml';
    }
}
