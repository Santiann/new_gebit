...
<div class="row">
    ...
    <div class="{{ (count($comboEmpresa)??0) == 1 ? "col-md-4" : "col-md-3" }}">
        <div class="form-group {{ $errors->has('a013_id_contrato') ? 'has-error' : ''}}">
            ...
            {!! Form::select('a013_id_contrato',isset($compromisso) ? [$compromisso->Contrato_belongsTo->a013_numero_contrato] : [],null,array('class' => 'form-control select2 ','id'=>'a013_id_contrato', $readonly??"", 'autocomplete' => 'off'))!!}
    ...
    <div class="{{ (count($comboEmpresa)??0) == 1 ? "col-md-4" : "col-md-3" }}">
        <div class="form-group {{ $errors->has('a005_id_empresa_cli_for') ? 'has-error' : ''}}">
            ...
            {!! Form::select('a005_id_empresa_cli_for',isset($compromisso) ? [$compromisso->empresa] : [],null,array('class' => 'form-control select2 ', $readonly??"",'id'=>'a005_id_empresa_cli_for', 'required' => 'required','autocomplete' => 'off'))!!}
    ...
    <div class="col-md-2">
        <div class="form-group {{ $errors->has('a022_valor_pagar') ? 'has-error' : ''}}">
            ...
            {!! Form::text('a022_valor_pagar', null, ['class' => 'form-control moneyMaskCuston','autocomplete' => 'off', $readonly??"",'required' => 'required']) !!}
    ...
    <div class="{{ (count($comboEmpresa)??0) == 1 ? "col-md-4" : "col-md-3" }}">
        <div class="form-group {{ $errors->has('a022_categorias') ? 'has-error' : ''}}">
            {!! Form::label('a022_categorias', 'Categoria', ['class' => 'control-label']) !!}
            {!! Form::select('a022_categorias[]',$comboCategoria_contrato, isset($compromisso->a022_categorias) ? json_decode($compromisso->a022_categorias) : 0,array('class' => 'form-control jmultipleselect2 ','id'=>'a022_categorias', 'required' => 'required','autocomplete' => 'off', 'multiple' => 'multiple'))!!}
            {!! $errors->first('a022_categorias', '<p class="help-block">:message</p>') !!}
            <div class="help-block with-errors"></div>
        </div>
    </div>
