...
<div class="box-footer BotesFixoTopo">
    ...
    <a href="{{ url($url)}}" class="btn btn-default btnCancelar">
        <i class="fa fa-ban"></i> Cancelar
    </a>
   
    <a href="{{ $login_url??"" }}" class="btn btn-default jaSouMembro" style="display: none;">
        {{ __('adminlte::adminlte.i_already_have_a_membership') }}
    </a>

    @if(($empresa->a004_dono_cadastro??1) == 1 or true)
    <button type="button" onclick="enviarFormulario()" class="btn bg-olive pull-right">
        <i class="fa fa-save"></i> <span class="jtxtSalvar">{{ isset($submitButtonText) ? $submitButtonText : 'Salvar' }}</span>
    </button>
    @else
...
<div class="box-body">
    {!! Form::hidden('a005_id_empresa', $empresa->a005_id_empresa??0, ['id'=>'a005_id_empresa','class' => 'form-control','autocomplete' => 'off', 'maxlength'=>'20']) !!}
    ...
    {!! Form::hidden('tipo_empresa', ($tipo??''), ['id'=>'tipo_empresa','class' => 'form-control','autocomplete' => 'off', 'maxlength'=>'50']) !!}
    ...
    <div class="tab-content">
        <div class="active tab-pane" id="form_geral">
            @if(!isset($hide_header_buttons))
                @include ('sistema.empresa.formGeral')
            @else
                @include ('sistema.empresa.formGeral', ['hide_header_buttons' => true])
            @endif
        </div>
        @if(!isset($hide_header_buttons))
        <div class="tab-pane" id="form_contato">@include ('sistema.empresa.formContato')</div>
        <div class="tab-pane" id="form_socio">@include ('sistema.empresa.formSocio')</div>
            @isset($editPage)
                <div class="tab-pane" id="form_docs">@include ('sistema.empresa.formDocs')</div>
            @endif
        @endif
    </div>
...
@push('js')
    <script>
        ...
        $(document).ready(function () {
            ...
            @if (isset($notificarContatos) && $notificarContatos)
        ...
        function validaClienteFornecedor()
        {
            ...
            if($("#a005_ind_empresa").prop("readonly"))
            {
                if("{{($empresa->a005_ind_empresa??"0")}}" == "1")
                {
                    indEmp = true;
                }
                else {
        ...
        function notificarAlteracaoContato()
        {
            ...
            $.ajax({
                url: "/notificarAlteracaoContato/{{ $empresa->a005_id_empresa }}",
                type: "POST",
                data: array,
                headers: {'X-CSRF-TOKEN': CSRF_TOKEN},
                success: function (response) {
